    ...
    static async getMessages(messageId) {
        let messages = await query("SELECT msgs.`chat-messagesId` as messageId, msgs.contactId, msgs.message, msgs.deliveryTime, msgs.authorId, msgs.forwarded_from, msgs.forward_chat " +
            ...
            if(messages.length > 0) {
                ...
                let message = new Message(messages[0].messageId, messages[0].contactId, messages[0].message, messages[0].deliveryTime, messageAuthor, [], messages[0].forwarded_from, messages[0].forward_chat)
                return message
            } else {
                return
            }
